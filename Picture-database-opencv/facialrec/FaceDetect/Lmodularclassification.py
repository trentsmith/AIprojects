...
class BSTNode:
    ...
    def insert(self, val):
        ...
        if self.val == val:
            return
        while self.first25!=None and self.second50!= None and self.third75!=None:        
            if 10<val < 15:
                self=self.first25
            if 0<val < 5:
                self=self.second50
            ...
            while self.first25==None and self.second50== None and self.third75==None:        
                if 10<val < 15:
                    self.first25=node(val)
                    return
                if 0<val < 5:
                    self.second50=node(val)
                    return
                if 5<val < 10:
                    self.third75=node(val)
                    return
    ...
    def find(self, val):
        ...
        if self.val == val:
            return
        while self.first25!=None and self.second50!= None and self.third75!=None:        
            if self.first25.val==self.val:
                return self.first25.val
            if self.second50.val==self.val:
                return self.second50.val
            if self.third75.val==self.val:
                return self.third75.val
            if 10<val < 15:
                self=self.first25
            if 0<val < 5:
                self=self.second50
...
#number array will always be the same
#numbers is array [1 1] where the first number is a representation of riemann zeta funnction skipping the individual numbers the seconde the offest of it.
